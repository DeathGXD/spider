...
class Handler(BaseHandler):
    ...
    @every(minutes=24 * 60)
    def on_start(self):
        self.conn = pymysql.connect(host="47.101.146.57", port=2018, user="root",password="Liuku!!!111",db="dm_report", charset='utf8')
    ...
    def on_result(self,result):
        ...
        sql = '''insert into toutiao_news(title,news_time,url,from_source,author,news_type,contents,image_url,check_status) values ('{}','{}','{}','{}','{}','{}','{}', '{}','{}');'''.format (result['title'].replace(chr(39), "\\'"),result['news_time'],result['url'],result['from_source'],result['author'],result['news_type'],result['content'].replace(chr(39), "\\'"),result['image_url'],'0')
        print(sql)
        cursor.execute(sql)
        self.conn.commit()
        cursor.close()
